...
public class StageData extends SubsystemBase {
	...
	public static GameStage getStage() {
		...
		if (gameTime < 30)
			...
			return GameStage.ShiftFour;

		else if (gameTime < 80) 
			return GameStage.ShiftThree;

		else if (gameTime < 105) 
			return GameStage.ShiftTwo;

		else if (gameTime < 130) 
			return GameStage.ShiftOne;

		else
			return GameStage.TransitionShift;
	}

	public static boolean canScore() {
		GameStage stage = getStage();
		
		if (stage == GameStage.Auto || stage == GameStage.Endgame)
			return true;

		else if (stage == GameStage.ShiftOne || stage == GameStage.ShiftThree) {
			if (m_alliance == m_firstInactiveAlliance)
				return false;
			else
				return true;
		}
